## Hi there 👋 I'm Khayrol Islam (0xmadtiger)

🛡️ Security Researcher | Ethical Hacker | Tool Builder  
💻 Linux (Arch) User | Automation Lover  
🌍 From Bangladesh 🇧🇩

---

### 🔭 What I’m Currently Working On
- 🔎 Web Recon & Vulnerability Automation Tools  
- ⚙️ CLI & GUI-based Security Tools (Python, Async)  
- 🕸️ WordPress & Web App Security Research  

---

### 🌱 What I’m Currently Learning
- Advanced Web Exploitation  
- LLM & AI for Security Automation  
- Reverse Engineering & Malware Analysis  

---

### 🧰 Tools & Technologies
- **Languages:** Python, Bash, PHP, JavaScript  
- **Security:** Burp Suite, Nuclei, Metasploit, nmap  
- **Frameworks:** PyQt6, Asyncio, httpx  
- **OS:** Linux  | Windows  

---

### 👯 I’m Looking to Collaborate On
- Open-source security tools  
- Bug bounty automation scripts  
- Red Team / CTF related projects  

---

### 💬 Ask Me About
- Web Application Security  
- Recon Automation  
- WordPress Vulnerabilities  
- Python for Hackers  

---

### 📫 How to Reach Me
- GitHub: [https://github.com/0xmadtiger](https://github.com/0xmadtiger)

---

### ⚡ Fun Fact
> I automate everything I can — if it’s manual, it’s slow 😄

---

⭐ **Check out my repositories and feel free to contribute!**
